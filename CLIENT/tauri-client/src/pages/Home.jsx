...
function Home() {
  ...
  const [streamData, setStreamData] = useState({ width: 1280, height: 720 });
  ...
  const handleConnect = async () => {
    ...
    try {
      ...
      const isRtsp =
        features.stream_protocol &&
        features.stream_protocol.toLowerCase().includes("rtsp");
      const activeProtocol = isRtsp ? "rtsp" : "mjpeg";
      setStreamProtocol(activeProtocol);

      let targetUrl = `http://${phoneIP}:${httpPort}/video`;
      if (isRtsp) {
        setRtspPort(features.rtsp_port || rtspPort);
        targetUrl = `rtsp://${phoneIP}:${features.rtsp_port || rtspPort}`;
      }

      updateStatus(
        `Starting ${activeProtocol.toUpperCase()} stream...`,
        "idle",
      );

      await invoke("start_sender", {
        source: activeProtocol,
        url: targetUrl,
      });
  ...
  const fetchSettings = async () => {
    ...
    try {
      ...
      if (settings.stream_protocol && settings.stream_protocol !== streamProtocol) {
        setStreamProtocol(settings.stream_protocol);
      }
      if (settings.supported_resolutions && settings.supported_resolutions.length > 0) {
        setResolutions(settings.supported_resolutions);
      }
    } catch (err) {
            ...
            <div className="space-y-4 pt-2 border-t border-slate-800">
              ...
              <div className="space-y-1.5">
                <div className="flex justify-between items-center">
                  ...
                  <span className="text-[9px] text-brand-400 font-mono">
                    {deviceSettings?.rotation || "auto"}
                  </span>
                </div>
                <select
                  value={deviceSettings?.rotation || "auto"}
                  onChange={(e) =>
                    sendControl(
                      `rotation=${encodeURIComponent(e.target.value)}`,
                  ...
                  className="w-full bg-slate-800 border border-slate-700 rounded-lg px-2 py-1.5 text-xs text-slate-300 outline-none focus:ring-1 focus:ring-brand-500"
                >
                  <option value="auto">Auto (Phone Gyro)</option>
              ...
              <div className="space-y-1.5">
                ...
                <select
                  ...
                  onChange={(e) => {
                    ...
                    setStreamProtocol(newProto);
                    sendControl(
                      `stream_protocol=${newProto}`,
                      `Codec: ${newProto.toUpperCase()}`
                    );
                  }}
                  className="w-full bg-slate-800 border border-slate-700 rounded-lg px-2 py-1.5 text-xs text-slate-300 outline-none focus:ring-1 focus:ring-brand-500"
                >
                  <option value="mjpeg">MJPEG</option>
                  <option value="rtsp">H.264 (RTSP)</option>
                </select>
              </div>
